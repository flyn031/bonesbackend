...
import { Prisma, ContactRole, ContactPerson } from '@prisma/client';
...
function isValidContactRole(role: string | null | undefined): role is ContactRole {
    ...
    try {
        // console.log('[isValidContactRole] Checking against ContactRole enum object:', ContactRole);
        if (!ContactRole) {
...
// Helper to check non-empty role strings
function isNonEmptyRole(role: any): role is ContactRole {
    return typeof role === 'string' && 
           role !== '' && 
           Object.values(ContactRole).includes(role as ContactRole);
}
...
export const createContactPerson = async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        const newContact = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
...
export const setPrimaryContact = async (req: AuthRequest, res: Response) => {
    ...
    try {
        const updatedContact = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
...
export const updateContactPerson = async (req: AuthRequest, res: Response) => {
    ...
    try {
        if (role !== undefined) {
            if (role === null || role === '') {
                ...
                // Fix for line 239 - use isNonEmptyRole instead of direct comparison
                if (isNonEmptyRole(role)) {
                    validatedRole = role;
                } else {
                    validatedRole = null;
    ...
    try {
        const resultContact = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
