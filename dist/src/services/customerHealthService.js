...
exports.calculateCustomerHealth = void 0;
// Use the actual exported prisma client instance
const prismaClient_1 = __importDefault(require("../utils/prismaClient")); // Corrected import path assuming prismaClient.ts is in ../utils
// Helper function to safely calculate total from items JSON
const calculateTotalFromItems = (itemsJson) => {
...
const calculateCustomerHealth = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const healthScores = customers.map(customer => {
            ...
            const overallScore = Math.round(recencyScore * weights.recency + frequencyScore * weights.frequency +
            ...
            if (daysSinceLastOrder > 90)
                insights.push(`Inactive: Last order >${Math.floor(daysSinceLastOrder / 30)} months ago`);
            if (customer.orders.length >= 5)
                insights.push(`Loyal (${customer.orders.length} orders)`);
            else if (customer.orders.length === 1)
                insights.push('New customer');
            if (growthScore > 75)
                insights.push('Spending trend: Increasing');
            else if (growthScore < 30)
                insights.push('Spending trend: Decreasing');
            if (frequencyScore > 75)
                insights.push('High purchase frequency');
            else if (frequencyScore < 25 && customer.orders.length > 1)
                insights.push('Low purchase frequency');
            if (monetaryScore > 75)
                insights.push('High average order value');
            else if (monetaryScore < 25 && customer.orders.length > 0)
                insights.push('Low average order value');
            if (potentialUpsell)
                insights.push('Potential for upsell');
            if (churnRisk === 'High')
                insights.push('High churn risk');
            else if (churnRisk === 'Low')
                insights.push('Low churn risk');
            if (insights.length === 0)
                insights.push('Standard customer profile'); // Default insight
            return {
                customerId: customer.id,
                name: customer.name,
                overallScore,
...
exports.calculateCustomerHealth = calculateCustomerHealth;
